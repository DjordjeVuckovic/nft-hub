...
export class EventsRepository {
	...
	async saveUserRegistrationEvent(
		...
		try {
			...
			if (error.code === 11000) {
				...
				const existing = await this.contractEventModel
					.findOne({
						transactionHash,
						logIndex,
					})
	...
	async isUserRegistered(address: string): Promise<boolean> {
		const registration = await this.contractEventModel.findOne({ eventType: 'UserRegistered', user: address }).exec();
		return !!registration;
	}

	async getUserNFTs(address: string): Promise<ContractEventDocumentType[]> {
		return this.contractEventModel.find({ eventType: 'NFTMinted', to: address }).sort({ blockNumber: -1 }).exec();
	}
	...
	async getLatestFees(): Promise<ContractEventDocumentType | null> {
		return this.contractEventModel.findOne({ eventType: 'FeesUpdated' }).sort({ blockNumber: -1 }).exec();
	}
}
